...
namespace AuthenticatedSchoolSystem.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        {
            _ = await UserManager.SetTwoFactorEnabledAsync(User.Identity.GetUserId(), true);
            ApplicationUser user = await UserManager.FindByIdAsync(User.Identity.GetUserId());
            if (user != null)
            {
                await SignInManager.SignInAsync(user, false, false);
        ...
        {
            _ = await UserManager.SetTwoFactorEnabledAsync(User.Identity.GetUserId(), false);
            ApplicationUser user = await UserManager.FindByIdAsync(User.Identity.GetUserId());
            if (user != null)
            {
                await SignInManager.SignInAsync(user, false, false);
        ...
        public async Task<ActionResult> ManageLogins(ManageMessageId? message)
        {
            ...
            System.Collections.Generic.IList<UserLoginInfo> userLogins =
                await UserManager.GetLoginsAsync(User.Identity.GetUserId());
            System.Collections.Generic.List<AuthenticationDescription> otherLogins = AuthenticationManager
